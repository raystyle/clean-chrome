...
- 2026-09-12 **Release 分发产物落地并部署（时记 P0001,现 REQ-002）**：Release 全编验收全绿（无参数 9222 + about:blank 首页 + WS 零对话框 + 开关入 dll）,`tools\deploy-release.py` 部署自包含目录至 C:\browse-rs\chromium-152.0.7977.84（含 SxS manifest 与 initial_preferences）
...
- 2026-09-15 **console 参数预览抑制（D02-8,S006）**：console.* 与未捕获异常经 CDP 上报不再急切序列化对象数据进协议 payload（50 锚 34 文件,首入 v8/ 树）;实测通道图定档：152 的 preview 不调 getter/Proxy 陷阱,文本通道与 stock 一致（非差分,保持原样防反造差分）;四端验收矩阵全绿（本机 Dev/browse-rs 部署沙箱态/lan-mac/lan-ubuntu）;顺带修 M027（browse-rs AppContainer ACE,deploy 脚本内置自愈）;M024-M026（bh Node 环境探针坑/PS 管道换行污染/git apply 子目录 skip）入库
- 2026-09-15 **S007 外部复核**：palimpsest#3 的 Runtime.enable 检测向量在 stock Chrome 155/152 实测已死（Proxy 陷阱由无条件文本构建通道触发,与 CDP 附着无关）;六项改进意见落 S007,裁定仅内部留存
- 2026-09-15 **文档体系迁移 dev-evo 五节合同（REQ-011,ADR-0007）**：AGENTS 重写五节合同;docs 新增 adr（7 篇）与 requirements（11 篇,PRD 全量迁入带 trace）;R001/G001 迁入 guides 并新建 G002 补丁工作流;MISTAKES 27 条分流进 R001 十节与 G002 四节（M0xx 行号保留）;根四原语与 INDEX、proven/mistakes/references/guide 退役,全仓引用替换;check.py 加 mdrules.py 入 tools 为提交前门禁（PE-01至13 全绿）
